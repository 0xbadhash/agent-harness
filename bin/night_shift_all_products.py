...
def write_summary(vault: Path, when: datetime, rows: list[dict], dry_run: bool) -> list[str]:
    ...
    lines = [
        ...
        f"**Schedule:** 03:15 Asia/Hong_Kong (harness timer)",
        f"**SoT:** `{HARNESS_ROOT}`",
        "",
    ...
    lines.extend(
        [
            ...
            "## Recommendations",
            "",
            f"1. Open each product vault `01-Projects/<label>/TODO.md` for checkboxes.",
            f"2. Fix ❌ products before `/execute_dev` on that repo.",
