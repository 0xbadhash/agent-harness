---
name: anti_slop_design
description: >
  Enforce the pols.dev anti-slop design law on every UI/UX/frontend design or
  build task. Load before designing interfaces, during implementation, and as a
  mandatory pre-ship audit. Triggers: /anti_slop_design, /anti-slop, design UI,
  landing page, hero, marketing site, React/Next frontend, "make it pretty",
  CSS redesign, polish UI, avoid AI slop.
user-invocable: true
...
**Harness skill:** load this skill **every time** you design or build any interface.

## When this skill fires

- User asks for UI, landing page, marketing site, dashboard chrome, hero, CTA, theme, or visual polish  
- Any frontend work that chooses type, color, layout, motion, or component chrome  
- Explicit: `/anti_slop_design`, `/anti-slop`, "no AI slop", "de-slop"  
- Before calling any UI work **done** or shipping a PR that touches presentation

## Protocol (mandatory)

### 1. Confirm before you begin

Tell the user in plain words that you have **read this entire skill (full law below)** and understood every point, and **promise** that at the end you will walk **every** point, find and fix UI errors, and hand over your best work.

### 2. Defaults vs user word

Everything in the law is the **DEFAULT**. Follow it religiously **unless** the user gives a specific, unambiguous instruction that conflicts (then their direction wins 100%). Absent that, this file is law. When your instinct conflicts with the law, **the law wins**.

### 3. While building

Keep the tells and the premium craft sections in mind. Prefer real choices over presets. One cohesive world: one palette, one type voice, one signature artifact.

### 4. Before ship — prove it

Walk the full law **point by point** (tells + layouts + deeper tells + premium craft + signature + field notes). Fix every shortfall. Do not mark done until that re-check is complete. Report briefly which major categories you re-checked.

### 5. Output contract

- No secret keys in examples  
- Prefer self-hosted / distinctive type over Google default shelf when choosing fonts  
- Prefer real imagery or authored illustration over crude CSS "illustrations"  
- Prefer purposeful motion over dead static UI; never gate content on opacity-0 entrance

## Slash / aliases

| Invoke | Meaning |
|--------|---------|
| `/anti_slop_design` | Full protocol + law |
| `/anti-slop` | Same |
| "de-slop this" | Audit existing UI against the law and fix |

## Related harness skills

- `execute_dev` — implement under TDD; load this skill when the task is UI  
- `cross_review` / `pr_review` — flag visual slop as a review finding when UI changed  
- Product frontend stacks vary; this skill is stack-agnostic

---

## Full law (authoritative body)

The remainder of this file is the **pols.dev anti-slop design law**. Treat every section as binding defaults.
